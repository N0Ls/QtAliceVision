...
FloatImageIORunnable::FloatImageIORunnable(const QUrl& path, int downscaleLevel, QObject* parent)
    : QObject(parent)
    , _path(path), _downscaleLevel(downscaleLevel)
{}
    ...
    : QQuickItem(parent)
{
    ...
    connect(&_surface, &Surface::sfmChanged, this, &FloatImageViewer::update);
}
...
QVector4D FloatImageViewer::pixelValueAt(int x, int y) const
{
    if (!_image)
    {
...
QSGNode* FloatImageViewer::updatePaintNode(QSGNode* oldNode, QQuickItem::UpdatePaintNodeData* data)
{
    ...
    QSGGeometryNode* rootGrid = nullptr;
    ...
    if (!root)
    {
        ...
        root->setFlags(QSGNode::OwnsMaterial);
        {
            ...
            rootGrid = new QSGGeometryNode;
            auto material = new QSGFlatColorMaterial;
            material->setColor(_surface.getGridColor());
            {
                // Vertexcount of the grid is equal to indexCount of the image
                auto geometryLine = new QSGGeometry(QSGGeometry::defaultAttributes_Point2D(), _surface.indexCount());
                geometryLine->setDrawingMode(GL_LINES);
                geometryLine->setLineWidth(2);

                rootGrid->setGeometry(geometryLine);
                rootGrid->setFlags(QSGNode::OwnsGeometry);
                rootGrid->setMaterial(material);
                rootGrid->setFlags(QSGNode::OwnsMaterial);
            }
            root->appendChildNode(rootGrid);
        }
    }
    else
    {
        ...
        rootGrid = static_cast<QSGGeometryNode*>(oldNode->childAtIndex(0));
    ...
    if (_surface.hasSubsChanged())
    {
        ...
        {
            // Vertexcount of the grid is equal to indexCount of the image
            rootGrid->geometry()->allocate(_surface.indexCount());
            rootGrid->markDirty(QSGNode::DirtyGeometry);
        }
        // Resize root
        if (root)
        {
    ...
    if (_imageChanged)
    {
        if (_surface.isLensDistortionViewerEnabled() || _surface.isPanoramaViewerEnabled())
        {
            updateSfmData = true;
        }
    ...
    if (root && !_createRoot)
    {
        updatePaintSurface(root, material, rootGrid->geometry(), updateSfmData);
    }
...
void FloatImageViewer::updatePaintSurface(QSGGeometryNode* root, QSGSimpleMaterial<ShaderData>* material, QSGGeometry* geometryLine, bool updateSfmData)
{
    ...
    // if (_surface.hasVerticesChanged()) // TODO: FACATMP
    {
        ...
        if (updateSurface)
        {
            ...
            Q_EMIT _surface.sfmChanged();
        }
    }

    // Draw the grid if Disto Viewer is enabled
    if (_surface.isLensDistortionViewerEnabled()) // && _surface.hasGridChanged())  // TODO: FACATMP
    {
        _surface.drawGrid(geometryLine);
    }
    else if (!_surface.isLensDistortionViewerEnabled())
    {
        // TODO : line width 0
